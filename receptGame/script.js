...
function saveGame() {
    const preparationAreaContent = Array.from(preparationAreaElement.children).map((el) => el.textContent);
    const gameState = {
        score,
        timeLeft,
        currentOrder,
...
function loadGame() {
    ...
    if (savedState) {
        ...
        startTimer(); // Restart the timer with the loaded time
        alert("Game Loaded!");
    }
    else {
...
function addIngredientToPreparationArea(ingredient) {
